...
3. **Location-Based Service Areas** ── Configure custom tax rates for the states/regions you service. All quotes and invoices will dynamically resolve the correct tax rate based on the specific work area address, eliminating static company-wide default tax rates.
...
2. ✉️ **Send Quote** ── Automatically creates a draft job and takes you straight to the Quote Builder with all customer details pre-filled.
...
- The "Send Quote" button is the fastest path from lead to revenue.`,lastUpdated:"2026-04-14",keywords:["inquiry","inquiries","portal","lead","customer request","website","booking","ticket","pending","dashboard"]},{id:"gs-admin-dashboard-triage",title:"Reorganized Admin Dashboard & Triage Workflow",category:"getting-started",content:`Your Admin Dashboard is organized into a top-to-bottom daily checklist to streamline dispatcher operations:

1. **Triage Inquiries & Change Requests (Top Box)** ── Immediate action items. Review website portal requests, voice call details, and quote change requests. Empty this top box daily by dismissing, converting to jobs, or revising quotes.

2. **Manage Quotes & Pipeline (Middle Box)** ── View live counts of Draft, Sent, Review, Approved, and Declined quotes. Click quote count cards to instantly open the Quotes List filtered by that status. The table lists the 5 most recent quotes; click any row to edit or view.

3. **Dispatch & Monitor Jobs (Bottom Box)** ── Track active work orders. View live counts of Unscheduled, Scheduled, In Progress, Completed, and Cancelled jobs. Click job count cards to view the filtered Jobs List. Click table rows to manage specific jobs.

4. **Operational Analytics (Bottom Row)** ── Track monthly revenue trends, job status distributions, and manage your field technician list.`,lastUpdated:"2026-06-04",keywords:["dashboard","workflow","checklist","triage","quotes","jobs","inquiries","change requests","dispatcher","daily operations"]},{id:"gs-active-modules",title:"Active Modules & Granular Features Settings",category:"getting-started",content:`Customize your DispatchBox workspace by enabling or disabling key operational modules and specific sub-features based on your business needs. You can configure these during onboarding or update them at any time from your settings.

**Major Categories & Granular Sub-Features:**

1. **Communications Hub**
   - **Email Client** ── Manage business inbox email sync directly inside DispatchBox.
   - **SMS & Texting** ── Send automated appointment reminders and message customers.
   - **AI Voice Phone Agent** ── Deploy Amy, your 24/7 autonomous phone receptionist.

2. **Invoicing & Estimates (Financials)**
   - **Proposals & Estimates** ── Draft professional quotes and request client signoffs.
   - **Digital Invoicing** ── Generate custom checkout links and process credit card payments.

3. **Inventory Tracking**
   - **Materials & Parts Catalog** ── Log warehouse inventory and monitor low-stock reorder alerts.
   - **Tool Fleet Audit** ── Track location, assignment, and status of specialized tools.

4. **Operations & Dispatch**
   - **Kanban Board** ── Monitor jobs using a highly visual drag-and-drop workflow pipeline.
   - **Calendar Console** ── Plan technician schedules on visual day, week, and month calendars.
   - **Dispatch Map** ── Track field technician GPS locations and optimize routing in real-time.

**Parent-Child Synchronization:**
The module settings toggle orchestrator automatically keeps parent categories and child sub-features in sync:
- **Disabling a Category:** Toggling off a parent category (e.g., Inventory Tracking) immediately disables all nested sub-features (Materials & Tools).
- **Enabling Sub-Features:** Enabling any sub-feature automatically activates its parent category. Conversely, if you disable *every* sub-feature under a category, the parent category switches off automatically to preserve visual clarity.

**Real-time Sidebar Category Folding:**
When a module or sub-feature is toggled off, all corresponding pages and links immediately disappear from the left sidebar. If a parent section becomes completely empty (e.g. all Dispatch features or all Inventory tools are disabled), the entire section header collapses to provide a beautifully clean, simplified workspace tailored specifically to your active features.

**Graceful Degradation & Manual Overrides:**
If you choose to disable feeding database modules, DispatchBox is engineered to degrade gracefully to ensure business continuity:
- **Custom Sourcing & POs:** When the Inventory/Materials module is disabled, the Purchase Orders & Procurement module transitions into a manual sourcing cockpit. A **"+ Add Custom Item"** inline builder form appears on PO search modals, allowing you to compile custom vendor PO drafts manually by typing in item names, custom SKUs, unit costs, and quantities.
- **Custom Quotes:** Quote Builders continue to let technicians type custom line-items and descriptions manually, even without active materials or parts catalog databases.`,lastUpdated:"2026-06-01",keywords:["modules","active","toggle","hide","show","onboarding","signup","comms","financial","inventory","purchase orders","sidebar","settings","background sync","granular","kanban","sms","email","voice","materials","tools","manual PO","custom item"]},{id:"jobs-calendar",title:"Using the Calendar",category:"jobs",content:`The Calendar view shows all scheduled jobs in a visual timeline.
...
**Single-Day Dispatch Grid:** To keep the calendar readable when dispatching multiple technicians, the grid displays a single active day at a time, side-by-side. Use the **Active Day** tabs (Monday to Friday) below the filter bar to switch days of the current week.

**Color Coding:** Jobs are color-coded by status ── blue (scheduled), yellow (in progress), green (completed), red (cancelled).

**Quick Create:** Click any empty time slot to create a new job at that time.`,lastUpdated:"2026-06-04",keywords:["calendar","schedule","drag","drop","view","day","week","month","active day","dispatch grid"]},{id:"jobs-status",title:"Job Statuses Explained",category:"jobs",content:`Jobs progress through these statuses:
...
**Status Tabs & Archived Jobs**
Filter jobs by status using the tabs: **All**, **Unscheduled**, **Scheduled**, **In Progress**, **Completed**, **Cancelled**, and **Archived**.
- The first six tabs display **active** jobs only.
- The **Archived** tab lists jobs that have been archived. Archiving helps you clean up your active dashboard without losing historical job records.

**Search, Priority & Sorting**
- **Search bar** ── Filter jobs instantly by customer name, address, job type, or description.
- **Priority pills** ── Quick-filter by All, Critical, High, Medium, or Low priorities.
- **Sorting** ── Click any column header (Priority, Customer, Type, Status, Tech, Duration, Age) to sort the list.

**Selecting & Managing Jobs (Bulk Actions)**
You can select multiple jobs by checking the boxes on the left side of the table rows:
- Check the **master checkbox** in the header to select all jobs currently shown.
- When one or more jobs are selected, a floating **Bulk Actions Bar** appears. Here, you can click **Archive Selected** (or **Unarchive Selected**) or permanently **Delete Selected**.
- Each action will prompt for confirmation before running.

**Individual Row Actions**
On the far right of each job row in the **Actions** column, you can perform quick individual actions:
- 👁️ **View Job** ── Opens the full job details.
- 👤 **Assign** (for unassigned jobs) ── Opens the smart technician assignment modal.
- 📦 **Archive / Unarchive** ── Archive active jobs or restore archived jobs back to active status.
- 🗑️ **Delete** ── Permanently remove the job from the system (requires confirmation).

**Smart Technician Assignment**
For unassigned jobs, click the **Assign** button or icon. This opens the **Smart Tech Assignment Modal** which ranks technicians using an AI-powered scoring engine based on skills, workload, availability, proximity, and certifications.`,lastUpdated:"2026-06-04",keywords:["jobs","work orders","list","table","filter","search","assign","unassigned","priority","sort","status","pending","archive","delete","bulk actions","select","work order management"]},{id:"jobs-one-click-booking",title:"Smart Scheduling & One-Click Booking",category:"jobs",content:`When a customer submits an inquiry or speaks to the AI voice agent, they often suggest dates and times that work for them (e.g., "next Tuesday morning" or "October 15th around 2 PM"). DispatchBox automatically parses these natural language requests into actionable scheduling chips.\\n\\n**How to use Smart Scheduling:**\\n1. Open the **Smart Tech Assignment Modal** by clicking "Assign" on any job that has customer availability.\\n2. At the top of the modal, you will see **Customer Suggested Times** displayed as clickable date chips.\\n3. Click any date chip to instantly jump to that specific day.\\n4. The "Available Slots" section will automatically refresh to show the specific time slots for the chosen day.\\n\\n**One-Click Booking:**\\nOnce you see the available time slots for the technician, hover over a slot. It will say "Book [Time]". Simply click the time slot button to instantly assign the technician to that exact time. This skips the extra confirmation step, allowing for lightning-fast dispatching.\\n\\n**Q: What if the customer just says "tomorrow"?**\\n**A:** DispatchBox's AI parser understands relative dates like "tomorrow" or "next week" based on when the ticket was created and accurately routes you to the correct day on the calendar.`,lastUpdated:new Date().toISOString().split("T")[0],keywords:["schedule","booking","one-click","assign","availability","parse","smart scheduling","customer suggested times","fast dispatch"]},{id:"inv-create",title:"Creating Invoices",category:"invoicing",content:`Invoices can be created in several ways:
...
These settings are saved automatically when you save the quote. You can preview exactly how the customer will see the quote by viewing the saved quote before sending it.`,lastUpdated:new Date().toISOString().split("T")[0],keywords:["quote","template","display","settings","presentation mode","single price","roll-up","category","tax","discount","percentage","fixed"]},{id:"inv-quote-change-requests",title:"Quote Change Requests & AI Revisions",category:"invoicing",content:`When a customer requests changes to a quote, DispatchBox automates the review and adjustment process to save you time:
...
Within the workspace, you can edit proposed line items manually, adjust markup, and modify the scope of work. Clicking **Save Changes** applies your edits to the root quote and clears the AI proposal, while keeping the quote in the review queue (\`tech_review\`) so it does not disappear from your dashboard. Once finalized, click **Send Quote** to deliver it to the customer. To bypass the AI workspace and access the classic manual editor, simply click the **Full Quote Editor** button inside the panel.`,lastUpdated:"2026-06-08",keywords:["quote","change request","revision","ai quote","dashboard","communication history","timeline","tech review","approve revision","regenerate ai"]},{id:"inv-quote-timeline-history",title:"Quote Activity Timeline & Detailed History",category:"invoicing",content:`DispatchBox maintains a comprehensive, visual audit trail for every service quote from inception to sign-off.

**What the Timeline Tracks:**
- **Quote Creation** ── Logs when the quote was originally created and whether it was auto-generated by the AI agent.
- **Delivery Status** ── Records when the quote was sent and how it was communicated (e.g., via Email, SMS, or Voice Callback).
- **Customer Views** ── Logs the exact timestamp when the customer opened and viewed the quote.
- **Revisions & Price Updates** ── Visualizes version changes side-by-side, detailing price adjustments between revisions.
- **Customer Notes & Change Requests** ── Displays notes submitted by the customer requesting adjustments, prompting technician review.
- **Sign-off / Decline** ── Captures approval signatures or decline reasons along with final pricing.
- **Deposit Payments** ── Records deposit details and payment methods.

**Visual Timeline Features:**
- **Collapsed Summary on the Line** ── Each timeline event shows a concise summary header (e.g., indicating AI generation, counts of line items like labor/materials, and communication status) so you can review history at a glance.
- **Expanded Detail View** ── Click any timeline row to expand and view the full details. For AI-generated quotes, this reveals a complete itemized breakdown of labor, materials, equipment, and travel, as well as the exact method and timestamp of customer communication.

**Q: Who can see technician messages in the timeline?**
**A:** Internal technician notes are visible to team members only. Customer-facing notes and system status changes are visible to both your staff and the customer.`,lastUpdated:"2026-06-08",keywords:["timeline","history","quote activity","line items","audit trail","communication method","sent via","email","sms","voice","revisions"]},{id:"inv-materials",title:"Managing Materials",category:"inventory",content:`Track materials and parts used on jobs:
...
5. **Availability (Service Only)** — "What days and times work best for you?" (Skipped for quote requests).
...
7. **Ticket Created** — Immediately after the caller confirms. Service requests generate Jobs, while Quote requests generate a Quote Inquiry ticket for human review.
...
• **Graceful address handling & validation fallback** — If address verification fails or the AI system is temporarily throttled, the agent contextually prompts specifically for the missing address or street number rather than reverting to a generic intake greeting.
• **Post-Intake Wrap-Up Protection** — Once a ticket is successfully created and the final recap has been read, the agent will gracefully end the call on any further input (such as ambient noise or confirmation statements), preventing infinite loops.
• **Multi-Correction & Representative Callback Handoff** — If the caller has to correct the AI multiple times (e.g. spelling corrections or rebuking wrong information), or if address validation fails twice, Amy automatically halts the automated intake. She politely reassures the caller that a human representative will call them directly to finalize details and schedule their service, immediately logging the ticket as a "Representative Callback" task and hanging up cleanly.
...
Set up your agent by clicking **AI Voice Agent** under the **Comms** section in the left sidebar navigation.`,lastUpdated:"2026-06-01",keywords:["AI","phone","agent","receptionist","call","answering","Amy","callback","email spelling","navigation","transfer","quote"]},{id:"addon-ai-admin",title:"AI Voice Management Dashboard",category:"addons",content:`Manage and troubleshoot your AI Voice Agent directly from the AI Voice Management dashboard (accessible to Site Admins under **Platform > AI Voice**).
...
3. Click **"Save and Resend"** to update their Customer Portal view.`,lastUpdated:"2026-04-03",keywords:["batch","invoice","multiple","edit","unlock","dispute"]},{id:"terms-conditions-rulesets",title:"Terms & Conditions Rule Sets",category:"invoicing",content:`DispatchBox includes a powerful, jurisdiction-aware Terms & Conditions engine that automatically generates legally compliant terms for every quote, customizable section-by-section per jurisdiction.
...
- Dispute resolution and governing law automatically reference the correct jurisdiction — not hardcoded

**Automatic Jurisdiction Detection:**
The system automatically detects the correct jurisdiction from the customer's address across ALL intake channels:
- **Web Portal / Email Intake** — extracted from the service address the customer submits
- **Phone Calls (AI Voice)** — extracted from the address collected during the call
- **SMS / Text Messages** — pulled from the existing customer record if they are a known customer
- **Manual Quote Creation** — auto-detected from the customer's address on file when creating a quote; can be overridden in the jurisdiction dropdown
- **AI Auto-Quote** — extracted from the job address, falls back to org service area settings
...
- Every subsequent customer in the same area uses the cached version instantly — no regeneration needed
...
**Configuring Rule Sets (Organization Settings → Legal & Terms):**

***Global Defaults:***
- **Company Legal Name** — Used in liability and indemnification clauses
- **Arbitration Venue** — Override the default dispute resolution location
- **Warranty Period** — Default workmanship warranty (days)
- **Quote Validity** — How many days quotes remain valid
- **Cancellation Notice** — Required advance notice for cancellations (hours)
- **Dispute Resolution Period** — Days for good-faith negotiation before arbitration
...
3. For each section you can:
   - **Use System Defaults** — zero-config, legally researched baseline
   - **Customize** — modify, add, or remove individual clauses
   - **Disable** — completely hide the section from quotes
   - **Reset** — revert all changes back to system defaults

***Customization Options:***
- **Uncheck a term** — removes that specific clause from quotes in this jurisdiction
- **Add Custom Clauses** — your clauses appear after the system defaults
- **Preview Terms** — see exactly how the merged terms will render on a quote
...
Germany (DE) terms include bilingual German/English text with BGB contract law, Widerrufsrecht (14-day right of withdrawal), Gewährleistung (statutory warranty), Handwerkskammer registration, and GDPR/DSGVO data protection notices.

**Q: Do I need to configure anything for T&C to work?**
**A:** No — system defaults provide comprehensive coverage out of the box. Only customize if you need specific changes for your business.

**Q: Are my customizations shared with other businesses?**
**A:** No — only the generic system defaults are shared (cached). Your org's customizations are private and stored separately.

**Q: How does the system know which state my customer is in?**
**A:** It extracts the state from the customer's service address (e.g., "123 Main St, Honolulu, HI 96815" → Hawaii). This works for all channels: web form, email, phone, and SMS. You can always override the jurisdiction manually when creating a quote.

**Q: What happens if I select the wrong jurisdiction on a quote?**
**A:** You can change the jurisdiction on the quote creation page. The system auto-detects from the customer's address when possible.`,lastUpdated:new Date().toISOString().split("T")[0],keywords:["terms","conditions","legal","jurisdiction","state","warranty","liability","arbitration","rule set","customize","override","germany","international","consumer protection","licensing","cache","shared","auto-detect","address"]},{id:"quote-activity-timeline",title:"Quote Activity Timeline & Approve-and-Pay",category:"invoicing",content:`Every quote includes a **Quote Activity Timeline** — a color-coded, expandable history of every interaction from creation to payment.

**Quotes Dashboard — Expandable Timeline:**
...
- Click the **▼ chevron** on any quote to expand its full communication timeline inline — no need to navigate away
...
- Status transitions appear as centered pills with "Waiting for…" indicators
...
- 📄 **Quote Created** (gray) — when the quote was first generated
- ✉️ **Quote Sent** (blue) — when emailed/shared with the customer
- 👁 **Quote Viewed** (purple) — when the customer first opened the link
- 💬 **Customer Message** (blue bubble, right-aligned) — customer's change requests or questions
- 💬 **Tech Reply** (amber bubble, left-aligned) — technician's responses
- ✏️ **Quote Revised** (amber) — when the quote was updated and re-sent
- ⚙️ **Status Change** (gray pill, centered) — workflow state transitions
- ✅ **Quote Approved** (green) — with signer name
- ❌ **Quote Declined** (red) — with reason
- 💳 **Deposit Paid** (emerald) — payment confirmation

**Step-by-Step Price History:**
- **Active Price Badges** — Each event header displays the active quote total at that specific step.
- **Revision Tracking** — Quote revisions explicitly list the pricing transition (e.g., "Version 1 ($100.00) ➔ Version 2 ($125.00)") so the history of changes is clear.
...
Customers can also choose **"Approve Only (pay later)"** to approve without paying immediately — the deposit CTA will appear on the quote page.
...
The timeline is also visible from the internal quote view, showing who sent what and when. When a customer requests changes, the tech review panel appears above the timeline with quick-reply and revise options.`,lastUpdated:"2026-06-05",keywords:["quote","timeline","activity","history","approve","pay","deposit","workflow","communication","message","reply","revised","status","stripe","expand","collapse","dashboard","save","scheduling","preferences","appointment slots","calendar","customer choice","price history","privacy","internal notes"]},{id:"customer-rate-cards",title:"Customer Rate Cards",category:"customers",content:`Offer VIP pricing to specific clients or commercial accounts:
...
- Enable Auto-Quote for full email to ticket to job to quote automation`,lastUpdated:"2026-04-24",keywords:["email","triage","inbound","smart","intake","form","spam","filter","unknown","trusted","auto-create","ticket","AI","classification","intake form","new customer","lead","onboarding","forwarding","forward","reply","proxy","reply-as","dispatch-box","personal email"]},{id:"email-batch-actions",title:"Multi-Select & Batch Email Actions",category:"communications",content:`DispatchBox makes managing your team inbox fast and efficient with powerful multi-select and batch operation tools.

**Selecting Multiple Emails:**
- **Far-Left Checkbox Column** ── Hover over or view any email row in the inbox list to see the selection checkbox on the far left.
- **Select All Checkbox** ── Click the master checkbox at the very top of the email list header to select all visible emails that match your current folder and mailbox filter.

**Elegant Contextual Actions Bar:**
When one or more emails are selected, the standard folder path header automatically transitions into a sleek contextual batch actions bar. The actions bar displays:
1. **Selected Count** ── Shows exactly how many emails are currently selected.
2. **📥 Archive / Unarchive** ── Move all selected emails to your Archive folder, or restore them back to the active Inbox.
3. **🗑️ Trash / Delete** ── Move selected emails to the Trash folder (soft delete), or permanently delete them from Firestore if you are already inside the Trash folder.
4. **✉️ Mark as Read / Unread** ── Quickly mark all selected emails as read or unread in a single batch operation.

**Automatic Deselection Safeguards:**
To prevent accidental batch operations on hidden messages, the system automatically clears all checkbox selections whenever you switch folders (e.g. from Inbox to Sent) or change mailbox filters.`,lastUpdated:"2026-06-01",keywords:["email","multi-select","select all","batch","trash","delete","archive","read","unread","inbox","folder","actions bar"]},{id:"inbound-voice-sms",title:"Inbound Calls & Text Messages",category:"communications",content:`When customers call or text your dedicated business number, DispatchBox automatically creates a job visible in your Job Intake Dashboard.
...
4. **In the background**, DispatchBox's AI analyzes the request and generates a draft quote with:
   ...
   - **Tax** — automatically resolved dynamically in the background based on the customer's service address
...
1. Go to **Organization Settings** → **Financial**
...
- **Tax Rate (%)** — Adjust the custom tax percentage inline to handle specific client jurisdictions. The system dynamically pre-fills the correct tax rate resolved based on the work area's address.
...
- **Location Tax Configuration** — Set up custom tax names and rates for specific states or regions in **Organization Settings → Financial**. For example, configure "HI" (Hawaii) with "GET" at 4.712% and "CA" (California) with "Sales Tax" at 8.25%.
- **Automatic Matching** — When a work location/address is entered or edited, the system instantly matches it against your configured regions and applies the matching rate.
- **AI-Powered Fallback Lookup** — If an address is outside your pre-configured regions, **Gemini AI** is automatically triggered to analyze the location and trade category, look up the standard applicable local tax rate and tax name, and apply it in real time.
- **Visual Source Badges** — The tax settings card displays a clear badge indicating where the tax rate was sourced:
  - **Settings Rate** (green) — Sourced directly from your configured service regions.
  - **AI Resolved** (blue) — Sourced dynamically using AI lookup. A tool tip provides the tax justification.
- **Shared Global Tax Database** — Once an AI-resolved tax rate for a state or region is generated and verified (i.e., not changed or overridden by a technician or customer), it is securely cached in a shared, global database collection (\`global_tax_rates\`). Any new customers or organizations creating jobs in that same state/region will instantly retrieve this verified shared rate without triggering a new AI query, speeding up load times and saving API token usage. No private customer, job, or organization details are ever shared.
- **Manual Overrides** — Technicians and dispatchers can fully customize and override the tax rate or toggle taxes inline on the quote panel at any time.
...
- The markup percentage comes from your organization's Rate Card settings`,lastUpdated:new Date().toISOString().split("T")[0],keywords:["ai quote","pricing","transparency","vendor","inventory","markup","price source","badge","stock","estimate","fallback","material","cost","google shopping","tool filtering","tape measure","location tax","tax rates","sales tax","GET","AI tax resolution","shared global tax rate"]},{id:"materials-procurement-backlog",title:"Job Materials Procurement & Backlog",category:"inventory",content:`DispatchBox's Job Materials Procurement Pipeline automates sourcing and purchasing for your approved quotes and work orders. Accessible via the **Purchase Orders** page under the **Materials Backlog** tab.
...
- **Forwarded Header** ── Standard forwarded headers detailing the original From, Date, Subject, and To recipients are automatically generated and appended at the top of the message body.`,lastUpdated:"2026-05-29",keywords:["forward","email forwarding","forwarding","thread","conversation thread","chronological","expand","collapse","attachments"]},{id:"quote-internal-navigation-and-messages",title:"Quote Dashboard Navigation & Timeline Messages",category:"invoicing",content:`DispatchBox makes managing quote communications simple and integrated for technicians, dispatchers, and customers:

## 1. Dashboard Layout for Authenticated Staff
When a logged-in technician, dispatcher, owner, or administrator opens a quote link (e.g., /quote/QUOTE_ID), the page automatically adapts. Instead of a standalone public view, the quote is embedded in the main dashboard layout, keeping the sidebar and top navigation menus fully available for quick navigation to other areas of the app.
...
2. Scroll to the **Quote Activity** section.
3. Type your note in the input textarea below the timeline.
4. Click **Send** to post. The note instantly appears in the chronologically ordered activity list.`,lastUpdated:"2026-06-05",keywords:["quote","navigation","menus","messages","notes","timeline","customer portal","communication","post-approval","discussion"]},{id:"upfront-payment-policy-rules",title:"Upfront Deposit Policies & Dynamic Max-Deposit Rules",category:"invoicing",content:`DispatchBox offers advanced deposit settings to help protect your business and automate upfront payments from customers.
...
4. Instead of choosing a single rule, you can check **multiple rules** to apply to your organization:
   - **Always Require Deposit** ── Requires a percentage-based deposit on all quotes.
   - **New Customers Only** ── Requires a percentage-based deposit if the customer has no past billing history.
   - **Quotes Over $ Threshold** ── Requires a percentage-based deposit only for quotes exceeding a set threshold.
   - **100% of Materials/Parts Cost** ── Requires a deposit matching the sum of all material items on the quote.
   - **Paid Estimate** ── Requires a flat upfront fee (for diagnostic/on-site evaluation) that will be deducted from the invoice.
...
- You can also choose **Custom Amount** to manually type a deposit dollar figure, or select **No Deposit Required** to clear the deposit entirely.`,lastUpdated:"2026-06-05",keywords:["deposit","upfront payment","down payment","policy","threshold","estimate fee","always require deposit","new customers","materials cost","highest deposit","auto-evaluate","manual override"]},{id:"quote-communication-history",title:"Quote & Job Activity Timeline (Chronological History)",category:"invoicing",content:`DispatchBox displays the entire history of a quote, job, and payment in a clean, chronological timeline sorted from the first step (top) to the latest (bottom).
...
- **Quotes** ── Directly inside the expanded quote rows or on the Quote details page.
- **Jobs** ── Expand any active job row inside the Jobs List to view its timeline instantly.
- **CRM Customer Profile** ── Expand any historical job in the Customer's Job Ledger to see its activity.
- **Customer Portal** ── Visible to customers inside their self-service portal under Job details.

## 2. Color-Coded Actions
Different types of events are highlighted with distinct colors so you can quickly distinguish them:
- **Payments & Invoices** ── Emerald green (e.g. Invoice sent, Payment received)
- **Sign-offs & Approvals** ── Forest green (e.g. Quote approved)
- **Delivery & Service Work** ── Amber or blue (e.g. Job scheduled, Work completed)
- **Customer Notes** ── Indigo (e.g. Feedback or questions sent by the client)
- **Technician Notes** ── Purple (e.g. On-site observations or internal comments)
- **Declined Items** ── Rose/Red (e.g. Quote declined)
- **System Events** ── Slate gray (e.g. Quote draft created)

## 3. Pricing History at Each Step
The timeline dynamically calculates and embeds the quote's active price at every milestone. A clear price badge (e.g., \`$120.00\`) is displayed in the accordion headers. During quote revisions, the timeline logs a detailed revision event showing the price change (e.g., \`Version 1 → Version 2: Price changed from $100.00 to $120.00\`) to track commercial changes clearly.

## 4. Note Privacy Control (Internal vs. Public)
To protect company communications:
- **Staff-Facing Timelines** ── Display all communications, including internal technician notes and chat messages.
- **Customer-Facing Timelines** ── Filter out all free-form technician messages (\`isInternal={false}\`), keeping internal notes hidden, while preserving customer notes, status updates, and price revision events.`,lastUpdated:"2026-06-05",keywords:["timeline","history","quote history","job history","communication history","accordion","milestone","colors","notes","payments","approvals","price history","privacy","internal notes"]},{id:"ai-legal-jurisdictions",title:"AI-Generated Legal Terms & Custom Jurisdictions",category:"invoicing",content:`You can now generate custom legal Terms & Conditions for any country, state, or region globally using Gemini AI.

## 1. Adding a Custom Jurisdiction
To add a new custom region:
1. Go to **Organization Settings → Financial Settings**.
